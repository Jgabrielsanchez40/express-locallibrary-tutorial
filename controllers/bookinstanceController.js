...
exports.bookinstance_create_post = [
  ...
  (req, res, next) => {
    ...
    if(!errors.isEmpty()) {
      //These are errors. Render form again with sanitized values and error messages.
      Book.find({}, 'title')
        .exec(function(err, books) {
          if(err) { return next(err); }
          //Successful, so Render
    ...
    if(!errors.isEmpty()) {
      ...
      Book.find({}, 'title')
        .exec(function(err, books) {
          if(err) { return next(err); }
          //Successful, so Render
          res.render('bookinstance_form', { title: 'Update BookInstance', book_list: book, selected_book: bookinstance.book._id, errors: errors.array(), bookinstance: bookinstance });
        });
        return;
    }
    else {
